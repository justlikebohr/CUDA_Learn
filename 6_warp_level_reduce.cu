...
__device__ float WarpShuffle(float sum) {
	// __shf1_down_sync: 前面的thread向后面的thread要数据
	// __shf1_up_sync: 后面的thread要数据
	// 1. 返回前面的thread向后面的thread要的数据，比如__shf1_down_sync(0xffffffff, sum, 16)那就是返回16号线程，17号线程的数据
	// 2. 使用warp shuffle指令的数据交换不会出现warp在shared memory上交换数据时的不一致现象,这一点是由GPU driver完成,故无需任何sync,比如syncwarp
	// 3. 原先15-19行有5个if判断block size的大小,目前已经被移除，确认了一下__shf1_down_sync等warp shuffle指令可以handle一个block或一个warp的线程数量<32, 不足32会自动填充0
