...
// bug2 fix: cudaMemcpy/cudaMemcpyAsync的第三个参数是内存大小，而不是数据量，此处是写成了数据量
typedef float FLOAT;
...
int main()
{
	...
	int size_per_stream = nums_per_stream * sizeof(FLOAT); // 少写这里产生了bug2
	...
	/* !must allocate CPU pinned mem using cudaMallocHost */
	...
	for (int i = 0; i < nstreams; ++i) {
		...
		// bug: size_per_stream-->nums_per_stream，指针偏移应该加上偏移刷量而不是大小
		int start_per_stream = i * nums_per_stream;
		printf("size_per_stream=%d, start_per_stream=%d\n",size_per_stream,start_per_stream);
	...
	// when using ****Async, here we need use cudaDeviceSyncchronize to sync host and all streams of device.
	// when only need sync one stream device, use cudaStreamSynchronize, which is light-weight
