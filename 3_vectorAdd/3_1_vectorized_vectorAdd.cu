...
__global__ void mem_bw (float* A, float* B, float* C) {
	...
	for (int i = idx; i < MEMORY_OFFSET / 4; i += blockDim.x * gridDim.x) {
		...
		//尝试: 使用 nv ptx load global memory 指令，结果数据依然没变
		//结论: 大概是编译器优化:读了数据不做操作那就会不读
		float4 a1 = reinterpret_cast<float4*>(A)[i];
		//float a1 = LoadFromGlobalPTX(reinterpret_cast<float4*>(A) + i);
		float4 b1 = reinterpret_cast<float4*>(B)[i];
		//float b1 = LoadFromGlobalPTX(reinterpret_cast<float4*>(B) + i);
		float4 c1;
		// 测量显存带宽方法1:向量加法,248g/s
		c1.x = a1.x + b1.x;
		c1.y = a1.y + b1.y;
		c1.z = a1.z + b1.z;
		c1.w = a1.w + b1.w;
