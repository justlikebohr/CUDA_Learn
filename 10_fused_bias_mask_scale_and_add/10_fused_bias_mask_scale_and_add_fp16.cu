...
// 2. 难一点的融合算子可以见fusedDropout，来进一步体会融合算子的开发方法，总的来说，和此算子融合的思想一样
// 3. 此例子目前不能跑，正在fix
template<typename T>
...
struct MaskScaleAndElementwiseAddFunctor<half> {
	MaskScaleAndElementwiseAddFunctor(const uint8_t* mask, const half* add_val, float scale)
		: mask(mask), add_val(add_val), scale(scale) {}
	// half标量版本的MaskScaleAndElementwiseAdd，与L15区别不大，注意：有的GPU再有nvcc和cuda版本下，没有重载half*half的直接相乘版本，此时需要用hmul(half,half)来代替或者两个half强转为fp32来相乘再转回half，比如（half)((float)x * (float)y)
...
// biasAdd的输入两个，x.shape={rows, cols}, bias.shape={cols}，所以需要再L59通过除余循环读取这cols个bias
