...
__global__ void reduce_v0(float *d_in, float *d_out) {
	...
	// 每个线程在shared memory上跨index加另一个元素，知道跨度>线程数量
	// 此时一个block对d_in这块数据的reduce sum结果保存在id为0的线程上面
	for(int index = 1; index < blockDim.x; index *= 2) {
		// 注意! v0并没有warp divergence, 因为没有else分支,
		// 现在的v0和v1性能大体相似
		// v0慢的原因在于下一行使用了除余%, 除余%是个非常耗时的指令
...
int main(){
	...
	printf("allocated %d blocks, data counts are %d", GridSize, N);
